...
function to2(x){ const n = Number(x); return Number.isFinite(n) ? n.toFixed(2) : ''; }
const defaultState = {}; MODEL.inputs.forEach(inp => defaultState[inp.address] = typeof inp.default==='number'?inp.default:(parseFloat(inp.default)||0));
...
function makeEvaluator(state) {
  ...
  return function evalFormula(formula) {
    ...
    try { return new Function('state','getCell','fns',"return (" + compiled + ");")(state, getCell, fns); }
    catch (e) { console.warn('Eval error', e, 'Compiled:', compiled); return NaN; }
  }
}
...
const RX_LHS  = /Approx\.?\s*quantity\s*in\s*meter[s]?\s*of\s*LHS\s*Cable\s*from\s*conveyor\s*to\s*deluge\s*valve\s*control\s*panel/i;
const RX_DIST = /Distance\s*from\s*deluge\s*valve\s*to\s*main\s*hydrant\s*line\s*in\s*Meter/i;

// Split inputs
const basicInputs = MODEL.inputs.filter(i => i.row > basicStart && i.row < advStart);
let advInputs = MODEL.inputs.filter(i => i.row > advStart);

// If LHS not an input (formula), synthesize it as editable
let lhsAddr = null;
const hasLhsInput = advInputs.find(i => RX_LHS.test(i.label||'')) || basicInputs.find(i => RX_LHS.test(i.label||''));
if (!hasLhsInput) {
  const lhsFormula = MODEL.formulas.find(f => RX_LHS.test(f.label||''));
  if (lhsFormula) {
    const fake = {address: lhsFormula.address, label: lhsFormula.label, default: 0, row: lhsFormula.row};
    advInputs = [fake, ...advInputs];
    state[fake.address] = state[fake.address] ?? 0;
    lhsAddr = fake.address;
  }
} else {
  const found = advInputs.find(i => RX_LHS.test(i.label||'')) || basicInputs.find(i => RX_LHS.test(i.label||''));
  lhsAddr = found ? found.address : null;
}

// Reorder: place LHS ABOVE Distance
function orderAdvanced(list) {
  const lhsIdx = list.findIndex(i => RX_LHS.test(i.label||''));
  const distIdx = list.findIndex(i => RX_DIST.test(i.label||''));
  if (lhsIdx > -1 && distIdx > -1 && lhsIdx > distIdx) {
    const lhs = list.splice(lhsIdx,1)[0];
    list.splice(distIdx,0,lhs);
  }
  return list;
}
advInputs = orderAdvanced(advInputs);
...
function computeAll() {
  ...
  MODEL.formulas.sort((a,b)=>a.row-b.row).forEach(f => {
    // Skip overwriting LHS if we synthesized/are treating it as manual
    if (lhsAddr && f.address === lhsAddr) return;
...
function renderResults(st) {
  ...
  MODEL.formulas.forEach(f => {
    ...
    const v = document.createElement('div'); v.textContent = to2(st[f.address]);
...
function renderBOM(st) {
  ...
  for (let r = 29; r <= 47; r++) {
    ...
    if (!(typeof qty === 'number' && (Number.isNaN(qty) || qty === 0))) {
      ...
      const tdq = document.createElement('td'); tdq.textContent = String(Math.round(Number(qty)||0));
